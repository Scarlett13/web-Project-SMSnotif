...
$sql = $con->prepare('select * from siswa where nis= ?');
$sql->execute(["$_SESSION[idu]"]);
$rs = $sql->fetch();
$ids=$rs['ids'];
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            ...
                            $sqlj = $con->prepare('select * from guru where idk= ?');
...
// $rsa=mysql_fetch_array($sqla);
    $sqla = $con->prepare('select * from sekolah where ids= ? and tgl= ? and jam= ?');
    $sqla->execute(["$ids", "$tanggul", "$kol"]);
    $rsa = $sqla->fetch();
...
// $sqla=mysql_query("select * from absen where ids='$ids' and tgl='$tanggul' and jam='$kol'");
// $rsa=mysql_fetch_array($sqla);
$sqla = $con->prepare('select * from guru where ids= ? and tgl= ? and jam= ?');
$sqla->execute(["$ids", "$tanggul", "$kol"]);
$rsa = $sqla->fetch();
