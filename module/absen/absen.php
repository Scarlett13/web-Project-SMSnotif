                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
  					...
  					// 		$sqlj=mysql_query("select * from kelas where idk='$_SESSION[idk]'");
                            ...
                            $sqlj = $con->prepare('select * from KELAS where idk= ?');
                            $sqlj->execute("[$_SESSION[idk]"]);
                            $rsj = $sqlj->fetch();
							echo "Kelas $rsj[nama]";					
							$klas=$_GET['kls'];
$rg=10;
...
if($klas=="semua"){
    ...
    $sql = $con->prepare('select * from siswa');
    $sql->execute();
}else{
	// $sql=mysql_query("select * from siswa where idk='$_GET[kls]'");
    $sql = $con->prepare('select * from siswa where idk= ?');
    $sql->execute(["$_GET[kls]"]);
}
	while($rs = $sql->fetch()){
	// $sqla=mysql_query("select * from absen where ids='$rs[ids]' and tgl='$dt' and jam='$_GET[jam]'");
	// $rsa=mysql_fetch_array($sqla);
	// $conk=mysql_num_rows($sqla);
    $sqla = $con->prepare('select * from absen where ids= ? and tgl= ? and jam= ?');
    $sqla->execute(["$rs[ids]", "$dt", "$_GET[jam]"]);
    $conk = $sqla->rowCount();  
    ...
    $sqlw = $con->prepare('select * from KELAS where idk= ?');
    $sqlw->execute(["$rsw[id]"]);
    $rsw = $sqlw->fetch();

    $sqlb = $con->prepare('select * from sekolah where id= ?');
    $sqlb->execute(["$_SESSION[idk]"]);
    $rsb = $sqlb->fetch();
