                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                    <form method="get" role="form" action="././media.php?module=input_absen&jam=1">
                                <div class="col-lg-6">
...
<input type="hidden" name="jam" value="1">
  ...
  if($_SESSION['level']=="guru"){
    ...
    $sql = $con->prepare('select * from kelas where idk= ?');
    $sql->execute(["$_SESSION[idk]"]);
        
  }else{
	// $sql=mysql_query("select * from kelas");	
    $sql = $con->prepare('select * from kelas');
    $sql->execute();
  }
	while($rs = $sql->fetch()){
	// $sqla=mysql_query("select * from sekolah where id='$rs[id]'");
	// $rsa=mysql_fetch_array($sqla);
    $sqla = $con->prepare('select * from sekolah where id= ?');
    $sqla->execute(["$rs[id]"]);
    $rsa = $sqla->fetch();
